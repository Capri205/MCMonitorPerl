package MCMonitorPerl::SocketConnection;
...
sub mcping {
    ...
    if ( $socket ) {
        $c->log->debug("Socket connection established to $ip : $port");
        ...
        my $plength = read_var_int( $c, $socket );
        my $ptype = read_var_int( $c, $socket );
        my $dlength = read_var_int( $c, $socket );
